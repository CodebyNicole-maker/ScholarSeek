...
const CandidateSearch = () => {
  ...
  const saveCandidate = () => {
    ...
    try {
      ...
      loadCandidate(); // Fetch a new candidate after saving
    } catch (err) {
      console.error("⚠️ Error saving candidate:", err);
    }
  };
    ...
    <main>
      ...
      {loading ? (
        ...
        <div className="card">
          ...
          />
          <h2>
            {candidate.name || "No Name Provided"} <em>({candidate.login})</em>
          </h2>
          <p><strong>Location:</strong> {candidate.location || "Not available"}</p>
          <p><strong>Company:</strong> {candidate.company || "Not available"}</p>
          <p>
          ...
          <div className="button-container">
            <button className="reject" onClick={loadCandidate}>Next Candidate</button>
            <button className="accept" onClick={saveCandidate}>Save Candidate</button>
          </div>
        </div>
      ) : (
        <p>No candidates available.</p>
